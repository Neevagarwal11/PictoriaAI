...
const WEBHOOK_URL =
  process.env.SITE_URL || "https://fdf44c2a6aff.ngrok-free.app";
  ...
  async function validateUserCredits(userId: string) {
    ...
    if(credits <=0){
      throw new Error("Insufficient credits to start model training!")
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(webhookurl, "training webhook url");
